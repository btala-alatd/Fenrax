...
export function composePrompt(
  ...
  const direction = lookbook
    ? [
        ...
        notes,
        kidsCraft,
        `Study original ${theme.label.toLowerCase()} merch composition. Invent ${name}. Never copy a known logo or trademark.`,
        `Stay inside this ${theme.label.toLowerCase()} house. Invent ${name}. Never copy a known logo.`,
    ...
    : [
        ...
        "Isolated 2D graphic centered on a perfectly even matte #F2F3F5 field. The field is empty studio, not part of the design.",
        ...
        "One giant idea. Not a clipart collage. Edges must knock out clean against the even field.",
        "Registration ticks or crosshairs only if they are inked as part of the graphic, never as a gray canvas.",
        `Invent original ${name} merch. Study ${theme.label.toLowerCase()} composition. Never copy a known logo or trademark.`,
